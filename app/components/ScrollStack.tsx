...
const ScrollStack = ({
  ...
  const getScrollData = useCallback(() => {
    if (useWindowScroll) {
      ...
      const cHeight = scroller ? scroller.clientHeight : 0;
      return {
        scrollTop: sTop,
  ...
  const measureLayout = useCallback(() => {
    ...
    // 2. Re-apply transforms temporarily
    cardsRef.current.forEach((card, i) => {
      if (card && cachedStyles[i]) {
        card.style.transform = cachedStyles[i].transform;
        card.style.filter = cachedStyles[i].filter;
      }
    });
